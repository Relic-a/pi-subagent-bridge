...
const server = new Server(
  { name: "pi-subagent-bridge", version: "0.2.5" },
  { capabilities: { tools: {} } },
);
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      ...
      description:
        "Return a bounded coordinator-ready snapshot of a run: state, recent activity, changed-file counts, and an event cursor. Use this before steering; it never exposes SQL.",
      inputSchema: {
        type: "object",
        required: ["run_id"],
        properties: { run_id: { type: "string", format: "uuid" } },
    ...
    },
    {
      ...
      description:
        "Read a bounded, sanitized, cursor-based run event feed for a specific run. Use after pi_get_run_status only when its snapshot is insufficient.",
      inputSchema: {
        type: "object",
        required: ["run_id"],
        properties: {
    ...
    },
    {
      name: "pi_steer",
      description:
        "Inject a bounded follow-up instruction into a running Pi RPC session without stopping or restarting it. Use pi_get_run_status first. Acknowledgement confirms RPC acceptance, not that the agent acted on the instruction.",
      inputSchema: {
        type: "object",
  ...
  try {
    switch (name) {
      ...
      case "pi_get_run_status":
        return jsonResult(
          manager.getRunSnapshot(RunIdSchema.parse(args ?? {}).run_id),
        );
      case "pi_get_run_events": {
        const parsed = EventsSchema.parse(args ?? {});
        return jsonResult(
          manager.getRunEvents(parsed.run_id, parsed.after, parsed.limit),
        );
      }
      case "pi_steer": {
