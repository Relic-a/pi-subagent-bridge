...
Clone this repository, install the plugin's runtime dependencies, and add the local marketplace:

```bash
git clone https://github.com/Relic-a/pi-subagent-bridge.git
cd pi-subagent-bridge
npm ci --omit=dev --prefix plugins/pi-subagent-bridge/server
...
To update, pull the repository, reinstall runtime dependencies, and reinstall the plugin:
