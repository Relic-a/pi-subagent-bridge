...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      name: "pi_start",
      description:
        "Start one isolated Pi RPC subprocess and return a stable run_id immediately.",
      inputSchema: {
        type: "object",
    ...
    },
    {
      name: "pi_wait",
      description:
        "Wait once for a Pi run's terminal agent_end event. This is not a polling API and request cancellation does not stop the Pi run.",
      inputSchema: {
        type: "object",
        required: ["run_id"],
    ...
    },
    {
      name: "pi_get_run",
      description:
        "Return diagnostic state for recovery and debugging. Not the normal waiting mechanism.",
      inputSchema: {
        type: "object",
        required: ["run_id"],
        properties: { run_id: { type: "string" } },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case "pi_wait":
        return jsonResult(
          await manager.wait(RunIdSchema.parse(args ?? {}).run_id),
        );
      case "pi_stop":
        return jsonResult(
