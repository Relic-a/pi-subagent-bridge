...
export class RunManager {
  ...
  async start(
    ...
    if (this.active.size >= maxConcurrentRuns) {
      throw new Error(
        `RUN_CONCURRENCY_LIMIT: maximum ${maxConcurrentRuns} active runs`,
      );
    }
    ...
    const client = new PiRpcClient({
      ...
      requestTimeoutMs: Number.parseInt(
        process.env.PI_RPC_REQUEST_TIMEOUT_MS ?? "120000",
        10,
      ),
    ...
    for (const record of pruned) this.removeWorkspace(record.workspace);
    ...
    client
      ...
      .then((data) => {
        ...
        if (!active || active.settled || active.state !== "starting") return;
  ...
  applyChanges(
    ...
    if (worktreeRoot) statusArgs.push(`:(exclude)${worktreeRoot}`);
  ...
  discardWorkspace(runId: string): { run_id: string; discarded: boolean } {
  ...
  private failRun(runId: string, error: string): void {
    const active = this.active.get(runId);
    if (!active || active.settled || TERMINAL_STATES.has(active.state)) return;
    if (active.state === "stopping") return;
    const sessionId = this.getRunSessionId(runId);
  ...
  private transition(
    ...
    if (!current) {
      if (tolerateRace) return false;
      throw new Error(`Unknown run_id: ${runId}`);
    }
    if (current === next) return true;
    if (!ALLOWED_TRANSITIONS[current].includes(next)) {
      if (tolerateRace && TERMINAL_STATES.has(current)) return false;
  ...
  private prepareWorkspace(
    ...
    try {
      ...
      if (useSnapshot) {
        ...
        const maxSnapshotBytes = Number.parseInt(
          process.env.PI_BRIDGE_MAX_SNAPSHOT_FILE_BYTES ?? "10485760",
          10,
        );
        const ignoredSnapshot =
          /(^|\/)(?:\.env(?:\..*)?|.*\.(?:pem|key|p12|pfx))$/i;
        for (const relative of untracked) {
          if (ignoredSnapshot.test(relative)) continue;
          const source = path.join(git.repoRoot, relative);
          const target = path.join(worktreePath, relative);
          const stat = fs.lstatSync(source);
          if (!stat.isFile() || stat.size > maxSnapshotBytes) continue;
  ...
  private messageForAgent(task: string, workspace: RunWorkspace): string {
    ...
    if (workspace.mode === "worktree") {
      instructions.push(
        "",
        `Bridge workspace note: you are running in an isolated git worktree at ${workspace.agent_working_directory}. Make code changes in that worktree. The coordinator will inspect changes with git using the branch, worktree, and patch artifacts returned by the bridge.`,
      );
    }
...
${task}`;
  ...
  private inferSessionId(runId: string): string | undefined {
    ...
    const inferable = [...this.active.values()].filter(
      (candidate) => !candidate.sessionId && !candidate.settled,
    );
    if (inferable.length > 1) return undefined;
    const snapshot = run.sessionSnapshot ?? new Map<string, number>();
  ...
  private finalizeOnce(
    ...
    finalAnswer: string,
    error?: string,
    sessionId = this.getRunSessionId(runId),
    terminate = true,
  ): boolean {
    const run = this.active.get(runId);
    ...
    if (run.forceTimer) clearTimeout(run.forceTimer);
    const storedAnswer =
      this.options.redactFinalAnswers && finalAnswer
  ...
  private removeWorkspace(workspace?: RunWorkspace): void {
    if (!workspace?.repo_root || !workspace.worktree_path) return;
    try {
      this.git(workspace.repo_root, [
        "worktree",
        "remove",
