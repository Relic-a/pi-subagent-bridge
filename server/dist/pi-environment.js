 ...
 */
export function preparePiEnvironment(env, bridgeDataDir, systemHome = os.homedir()) {
    ...
    const agentDir = path.resolve(explicitAgentDir ?? userAgentDir ?? path.join(bridgeDataDir, "pi-agent"));
    const sessionDir = path.resolve(nonEmpty(env.PI_CODING_AGENT_SESSION_DIR) ??
...
function findExistingUserAgentDir(env, systemHome) {
    const homes = [nonEmpty(env.HOME), nonEmpty(systemHome)].filter((home, index, all) => home !== undefined && all.indexOf(home) === index);
